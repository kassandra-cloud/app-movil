...
import com.example.proyecto.data.ComentarioCreateRequest
...
class ForoViewModel : ViewModel() {
    ...
    val postError   = mutableStateMapOf<Int, String?>()               // pubId -> error último
    ...
    fun cargar(token: String) {
        ...
        viewModelScope.launch {
            try {
                val data = ApiClient.foroApi.listar("Token $token") // usa "Bearer" si corresponde
                uiState = UiState(cargando = false, publicaciones = data)

                // arranca auto-refresh para cada publicación
                data.forEach { startAutoRefresh(token, it.id) }

            } catch (e: Exception) {
                Log.e("FORO", "listar()", e)
                uiState = uiState.copy(cargando = false, error = e.message)
            }
        }
    }
        ...
        viewModelScope.launch {
            try {
                val nuevo = ApiClient.foroApi.comentar(
                    "Token $token",
                    pubId,
                    ComentarioCreateRequest(contenido = body, parent = parentId)
                )
                // agrega al final localmente
                val prev = comentarios[pubId] ?: emptyList()
                comentarios[pubId] = prev + nuevo
                // y recarga desde servidor para no perder hilos
